                ...
                <table class="table table-striped table-bordered alt-pagination table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><a href="{{ route('admin.order_collect.list', [$row->batch_pk]) }}" class="btn btn-md btn-info" title="SENT FOR MOBILE DISPATCH-{{ $row->order_dispatched }} TOTAL-{{ $row->order_count }}">{{ $row->order_dispatched }} / {{ $row->order_count }}</a></td>
                            <td class="text-center"><a href="{{ route('admin.item_collected.list', [$row->batch_pk]) }}" class="btn btn-md btn-info" title="COLLECTED-{{ $row->item_count_collected ?? 0  }} TOTAL-{{ $row->item_count ?? 0 }}">{{ $row->item_count_collected ?? 0 }} / {{ $row->item_count }}</a></td>
