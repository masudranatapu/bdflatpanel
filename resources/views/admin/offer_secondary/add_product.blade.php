...
@section('title')
Add new product to secondary list
@endsection
@section('page-name')
Add new product to secondary list
@endsection
        ...
        <div class="col-md-12">
            <div class="card card-success">
                <div class="card-content collapse show">
                    <div class="card-body">
                        {!! Form::open([ 'route' => ['admin.offer_secondary.store_product'], 'method' => 'post', 'class' => 'form-horizontal', 'files' => true , 'novalidate']) !!}
                            ...
                            <div class="col-md-2">
                                <div class="form-group with-border">
                                    <div class="controls" id="scrollable-dropdown-menu2">
                                        <button type="button" class="btn btn-primary  btn-sm search_mother_btn mt-2" data-toggle="modal" data-backdrop="true" data-target="#variant-modal" data-url="{{ route('admin.offer_secondary.add_product',['id' => $row->PK_NO]) }}" data-multiple="1" >Product Finder</button>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-actions text-center mt-3">
                                    <a href="{{ route('admin.offer_secondary.list') }}">
                                                    ...
                                                    @foreach($row->secondaryDetails as $key => $crow)
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                ...
                                                                <a href="{{ route('admin.offer_secondary.deleteproduct', [$crow->PK_NO]) }}" class="btn btn-xs btn-danger mr-1" onclick="return confirm('Are you sure you want to delete the product with it\'s variant product ?')" title="DELETE"><i class="la la-trash"></i></a>
                                                                @endif
                                                            </td>
                                                        </tr>
                                                    @endforeach
...
var pageurl = `{{ route('admin.offer_secondary.productlist') }}`;
        $.ajax({
            type    : 'POST',
            url     : pageurl,
            async   : true,
