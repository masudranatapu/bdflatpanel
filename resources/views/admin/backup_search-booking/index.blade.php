                        ...
                        <div class="card-header">
                            @if(hasAccessAbility('new_booking', $roles))
                                @if(isset($info['type']) && ($info['type'] == 'customer'))
                                    <a href="{{route('admin.booking.create',['id' => $info['PK_NO'], 'type' => 'customer' ])}}" class="btn btn-round btn-sm btn-primary text-white"><i class="ft-plus text-white"></i> @lang('booking.role_create_btn')</a>
                                @elseif(isset($info['type']) && ($info['type'] == 'reseller'))
                                    <a href="{{route('admin.booking.create',['id' => $info['PK_NO'], 'type' => 'reseller' ])}}" class="btn btn-round btn-sm btn-primary text-white"><i class="ft-plus text-white"></i> @lang('booking.role_create_btn')</a>

                                @else
                                    <a href="{{route('admin.booking.create')}}" class="btn btn-sm btn-primary text-white"><i class="ft-plus text-white"></i> @lang('booking.role_create_btn')</a>
                                    ...
                                    <table class="table table-striped table-bordered alt-pagination table-sm" id="indextable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="badge {{ (date($current_time) > date($row->EXPIERY_DATE_TIME)) ? 'border-red red' : 'border-success success' }} round badge-border">
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    @if(hasAccessAbility('edit_booking', $roles))
                                                        <a href="{{ route('admin.booking.edit',$row->PK_NO) }}" class="btn btn-xs btn-info" ><i class="la la-edit"></i></a>
                                                    @endif
                                                    @endif

                                                    @if(hasAccessAbility('view_booking', $roles))
                                                    <a href="{{ route('admin.booking.view',$row->PK_NO) }}"  class="btn btn-xs btn-success"><i class="la la-eye"></i></a>
                                                    @endif
                                                    {{-- @if($row->BOOKING_STATUS < 50)
                                                    @if(hasAccessAbility('edit_booking', $roles))
                                                        <a href="{{ route('admin.booking_to_order.book-order',$row->PK_NO) }}" class="btn btn-xs btn-azura" title="PROCEED TO ORDER"><i class="ft-check-circle"></i></a>
                                                    @endif
                                                    @endif --}}
                                                    @if($row->BOOKING_STATUS < 50)
                                                    @if(hasAccessAbility('delete_booking', $roles))
                                                        <a href="{{ route('admin.booking.delete', $row->PK_NO) }}" onclick="return confirm('Are you sure you want to delete ?')" class="btn btn-xs btn-danger"><i class="la la-trash"></i></a>
                                                    @endif
                                                    @endif
                                                    @if(hasAccessAbility('edit_booking', $roles))
                                                    <a href="{{ route('admin.payment.create', [$row->F_CUSTOMER_NO, 'customer' ]) }}" class="btn btn-xs btn-info" title="ADD NEW PAYMENT"><i class="la la-usd"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endif
