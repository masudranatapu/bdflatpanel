                        ...
                        @if(isset($rows) && count($rows) > 0 )
                            @foreach($rows as $key => $row )
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.booking_to_order.book-order-view', ['id' => $row->F_BOOKING_NO ?? 0 ]) }}">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.revert_dispatch.dispatch',[$row->F_BOOKING_NO]) }}" onclick="return confirm('Are you sure?')" class="btn btn-xs btn-primary mb-05" title="REVERT BACK"><i class="la la-exchange"></i></a>
                                        @if ($row->order->IS_SELF_PICKUP == 1)
                                        <a href="{{ route('admin.order.dispatch',['id'=>$row->F_BOOKING_NO,'dispatch?type=cod_rtc']) }}" class="btn btn-xs btn-info mb-05" title="VIEW"><i class="la la-eye"></i></a>
                                        @else
                                        <a href="{{ route('admin.order.dispatch',['id'=>$row->F_BOOKING_NO,'dispatch?type=rts']) }}" class="btn btn-xs btn-info mb-05" title="VIEW"><i class="la la-eye"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
