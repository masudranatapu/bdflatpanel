...
class OrderAbstract implements OrderInterface
{
    ...
    {
            $regular_price = Stock::selectRaw('(SELECT IFNULL(AVG(REGULAR_PRICE),0) from INV_STOCK where IG_CODE = '. '"' .$ig_code. '"' .' and F_BOOKING_NO = '.$f_booking_no.')')->limit(1)->getQuery();
            $install_price = Stock::selectRaw('(SELECT IFNULL(AVG(INSTALLMENT_PRICE),0) from INV_STOCK where IG_CODE = '. '"' .$ig_code. '"' .' and F_BOOKING_NO = '.$f_booking_no.')')->limit(1)->getQuery();
    ...
    public function findOrThrowException($PK_NO,$type)
    {
        ...
        if($data['order']->IS_CANCEL == 1){
            ...
            // dd($data['booking_details_canceled']);
        }else{
...
/*
    ...
    public function postDefaultOrderPenalty($request,$id)
    {
        ...
        $orders =  Order::select('PK_NO','IS_RESELLER','ORDER_BALANCE_USED','ORDER_ACTUAL_TOPUP')
        ->where('F_BOOKING_NO',$id)
        ->first();
        ...
        DB::commit();
        return $this->formatResponse(true, 'Penalty successfully done !', 'admin.order_default_action.list','');
    }
    ...
    public function postCancel($id,$request)
    {
                ...
                if($role_id->F_ROLE_NO == 1){
                    ...
                    if($cancel_fee > 0 ){
                        if($order->ORDER_ACTUAL_TOPUP != $order->ORDER_BUFFER_TOPUP){
                            $msg = 'Order payment need to verify firstly';
        ...
        DB::commit();
        return $this->formatResponse(true, $msg, 'admin.order.list');
